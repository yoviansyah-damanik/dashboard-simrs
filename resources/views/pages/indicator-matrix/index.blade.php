<x-content>
    ...
    <x-sirs.report-header title="Matriks Indikator Tahunan"
        subtitle="BOR, ALOS, BTO, TOI, NDR, GDR per bulan &mdash; seluruh rumah sakit" :profil="$profil" bulan=""
        :tahun="$tahun" />
            ...
            <table class="w-full text-sm divide-y divide-stroke dark:divide-strokedark">
                <thead>
                    <tr class="bg-gray-50 dark:bg-meta-4">
                        <th class="px-4 py-3 text-left font-black text-gray-500 uppercase tracking-widest sticky left-0 bg-gray-50 dark:bg-meta-4 z-10">
                            Indikator</th>
                        @for ($bulan = 1; $bulan <= 12; $bulan++)
                            <th class="px-3 py-3 text-center font-black text-gray-500 uppercase tracking-widest border-l border-stroke dark:border-strokedark">
                                {{ Str::limit(ucfirst(strtolower(\App\Helpers\SirsHelper::getMonthName($bulan))), 3, '') }}
                            </th>
                        @endfor
                        <th class="px-4 py-3 text-center font-black text-primary uppercase tracking-widest border-l-2 border-primary/30 bg-primary/5">
                            Tahun</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stroke dark:divide-strokedark">
                    @foreach ($indicators as $key => $meta)
                        <tr class="hover:bg-gray-50 dark:hover:bg-meta-4/30">
                            <td class="px-4 py-3 font-bold text-gray-700 dark:text-gray-300 sticky left-0 bg-white dark:bg-boxdark z-10">
                                {!! $meta['label'] !!}</td>
                            @for ($bulan = 1; $bulan <= 12; $bulan++)
                            ...
                            @endfor
                            <td class="px-4 py-3 text-center font-black text-primary border-l-2 border-primary/30 bg-primary/5">
