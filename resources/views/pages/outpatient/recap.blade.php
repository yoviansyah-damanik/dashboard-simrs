    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-bold uppercase tracking-widest">Monitoring kunjungan dan rekapitulasi poliklinik secara real-time.</p>
                </div>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-gradient-to-br from-indigo-600 to-violet-800 p-6 rounded-3xl shadow-xl relative overflow-hidden flex flex-col lg:flex-row items-stretch gap-6 group border border-white/10">
            ...
            <div class="relative z-10 text-white min-w-[200px] flex flex-col justify-center">
                <p class="text-sm font-black uppercase tracking-[0.2em] text-indigo-200 mb-2">Total Kunjungan</p>
                ...
                <div class="flex items-center gap-6 mt-6">
                    <div class="flex flex-col">
                        ...
                        <span class="text-2xl font-black text-blue-300">{{ number_format($this->overallStats['gender']['laki']) }}</span>
                    </div>
                    <div class="w-px h-8 bg-white/20"></div>
                    <div class="flex flex-col">
                        <span class="text-sm font-black text-indigo-200 uppercase tracking-widest">Perempuan</span>
                        <span class="text-2xl font-black text-pink-300">{{ number_format($this->overallStats['gender']['perempuan']) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="relative z-10 text-white flex-1 border-x border-white/10 px-8 hidden lg:block">
                <p class="text-sm font-black uppercase tracking-[0.2em] text-indigo-200 mb-4 text-center">Rincian Kelompok Usia & Gender</p>
                    ...
                    @foreach($this->overallStats['age_groups'] as $age)
                        <div class="flex flex-col p-2.5 bg-white/10 rounded-xl border border-white/10 backdrop-blur-sm group/card hover:bg-white/20 transition-all">
                            <div class="flex items-center justify-between mb-1.5">
                                <span class="text-sm font-black text-indigo-100 uppercase tracking-tighter truncate w-3/4">{{ $age->kelompok }}</span>
                                <span class="text-sm font-black bg-white/20 px-1.5 py-0.5 rounded-md">{{ number_format($age->total) }}</span>
                            </div>
                            <div class="flex items-center gap-2 pt-1.5 border-t border-white/5">
                                <div class="flex-1 flex items-center gap-1 justify-center">
                                    ...
                                    <span class="text-sm font-black text-blue-200">{{ number_format($age->laki) }}</span>
                                </div>
                                <div class="flex-1 flex items-center gap-1 justify-center">
                                    <span class="w-1.5 h-1.5 rounded-full bg-pink-400"></span>
                                    <span class="text-sm font-black text-pink-200">{{ number_format($age->perempuan) }}</span>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="relative z-10 text-white min-w-[200px] flex flex-col justify-center items-center text-center">
                <p class="text-sm font-black uppercase tracking-[0.2em] text-indigo-200 mb-2">Penyelesaian</p>
                ...
                <div class="mt-4 px-4 py-1.5 bg-white/10 rounded-full border border-white/10">
                    <span class="text-sm font-black text-indigo-100 uppercase tracking-[0.1em]">Total Rate Periode</span>
                </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="bg-white dark:bg-boxdark p-4 rounded-2xl border border-stroke dark:border-strokedark shadow-sm flex flex-col items-center text-center group hover:border-indigo-500 transition-all">
                <span class="text-sm font-black text-gray-400 uppercase tracking-widest mb-1">Baru</span>
                <div class="flex items-center gap-1.5">
                    <h4 class="text-2xl font-black text-indigo-600">{{ number_format($this->recapData->sum('pasien_baru')) }}</h4>
            ...
            <div class="bg-white dark:bg-boxdark p-4 rounded-2xl border border-stroke dark:border-strokedark shadow-sm flex flex-col items-center text-center group hover:border-gray-500 transition-all">
                <span class="text-sm font-black text-gray-400 uppercase tracking-widest mb-1">Lama</span>
                <div class="flex items-center gap-1.5">
                    <h4 class="text-2xl font-black text-gray-700 dark:text-gray-300">{{ number_format($this->recapData->sum('pasien_lama')) }}</h4>
            ...
            <div class="bg-white dark:bg-boxdark p-4 rounded-2xl border border-stroke dark:border-strokedark shadow-sm flex flex-col items-center text-center group hover:border-emerald-600 transition-all">
                <span class="text-sm font-black text-gray-400 uppercase tracking-widest mb-1">Sudah</span>
                <h4 class="text-2xl font-black text-emerald-600">{{ number_format($this->recapData->sum('sudah_periksa')) }}</h4>
                <p class="text-sm font-bold text-gray-400 uppercase">Checked</p>
            </div>
            <div class="bg-white dark:bg-boxdark p-4 rounded-2xl border border-stroke dark:border-strokedark shadow-sm flex flex-col items-center text-center group hover:border-red-500 transition-all">
                <span class="text-sm font-black text-gray-400 uppercase tracking-widest mb-1">Belum</span>
                <h4 class="text-2xl font-black text-red-600">{{ number_format($this->recapData->sum('belum_periksa')) }}</h4>
                <p class="text-sm font-bold text-gray-400 uppercase">Pending</p>
            </div>
        </div>
    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-meta-4">
                                <th
                                    class="px-6 py-6 text-sm font-black uppercase tracking-[0.2em] text-gray-400 border-b border-stroke dark:border-strokedark">
                                    Poliklinik</th>
                                <th
                                    class="px-6 py-6 text-sm font-black uppercase tracking-[0.2em] text-gray-400 border-b border-stroke dark:border-strokedark">
                                    Dokter</th>
                                <th
                                    class="px-4 py-6 text-sm font-black uppercase tracking-[0.2em] text-center bg-indigo-50/50 dark:bg-indigo-900/10 border-b border-stroke dark:border-strokedark text-primary">
                                    Total</th>
                                <th
                                    class="px-4 py-6 text-sm font-black uppercase tracking-[0.2em] text-center border-b border-stroke dark:border-strokedark">
                                    Status Pasien</th>
                                <th
                                    class="px-4 py-6 text-sm font-black uppercase tracking-[0.2em] text-center border-b border-stroke dark:border-strokedark">
                                    Pemeriksaan</th>
                        ...
                        <tbody class="divide-y divide-stroke dark:divide-strokedark">
                            @forelse($this->recapData as $item)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-meta-4/20 transition-all duration-300 text-base">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                    ...
                                    <td class="px-4 py-4 text-center bg-indigo-50/20 dark:bg-indigo-900/5">
                                        <span class="inline-flex items-center justify-center min-w-[2.5rem] px-2.5 py-1 rounded-xl bg-indigo-600 text-white text-sm font-black shadow-lg shadow-indigo-200 dark:shadow-none">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-center gap-4">
                                            <div class="flex items-center gap-1.5">
                                                <span class="text-sm font-black text-gray-400 uppercase">B:</span>
                                                <span class="text-blue-600 font-black">{{ number_format($item->pasien_baru) }}</span>
                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="text-sm font-black text-gray-400 uppercase">L:</span>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-center gap-4">
                                            <div class="flex items-center gap-1.5">
                                                <span class="text-sm font-black text-emerald-500 uppercase">S:</span>
                                                <span class="text-emerald-600 font-black">{{ number_format($item->sudah_periksa) }}</span>
                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="text-sm font-black text-amber-500 uppercase">B:</span>
